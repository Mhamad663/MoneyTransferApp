...
class TransferController extends Controller
{
    ...
    public function storeWallet(Request $r)
    {
        ...
        DB::transaction(function () use ($data, $sender, $senderWallet, $receiverWallet, $service, $fee, $total, $ref, $note, &$transfer) {
            $transfer = Transfer::create([
                ...
                'source'         => $senderWallet->wallet_id,
                'destination'    => $receiverWallet->wallet_id,
                'src_currency'   => 'USD',
                'dst_currency'   => 'USD',
                'amount_src'     => $total,
    ...
    public function approveTransfer($id)
    {
        ...
        DB::transaction(function () use ($transfer) {
            $senderWallet   = Wallet::where('wallet_id', $transfer->source)->first();
            $receiverWallet = Wallet::where('wallet_id', $transfer->destination)->first();
            ...
            TransferEvent::create([
                'transfer_id' => $transfer->id,
                'event'       => 'approved',
                'meta'        => '✅ Wallet transfer approved and completed by agent.',
            ]);
        });

        $transfer->user->notify(new TransferStatusUpdated(
            $transfer,
            '✅ Your wallet transfer of $' . number_format($transfer->amount_dst, 2) . ' has been approved and completed.'
        ));

        return back()->with('success', '✅ Transfer approved and balances updated.');
    }
