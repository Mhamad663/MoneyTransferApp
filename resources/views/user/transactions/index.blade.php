      ...
      <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
        @forelse($items as $row)
          <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/40">
            ...
            {{-- Refund column --}}
            <td class="py-2 px-3">
    @php $refund = $row->refundRequest; @endphp
    ...
    @if($refund)
        @php
            ...
            $badgeClasses = match ($status) {
                ...
                'flagged_fraud' => 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300',
        ...
        <span class="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-full {{ $badgeClasses }}">
            {{ $label }}
        </span>
    @else
